...
public class IfOnEdgeBounceBrickTest extends InstrumentationTestCase {
	...
	public void testBounceNorth() {
		...
		width = sprite.getCostume().getRelativeWidth();
		height = sprite.getCostume().getRelativeHeight();
		assertEquals("Wrong X-Position!", 0, sprite.getXPosition());
		...
		assertEquals("Width shouldn't change", width, sprite.getCostume().getRelativeWidth(), 1e-3);
		assertEquals("Height shouldn't change", height, sprite.getCostume().getRelativeHeight(), 1e-3);

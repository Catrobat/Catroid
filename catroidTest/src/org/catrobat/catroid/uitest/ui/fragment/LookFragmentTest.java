...
public class LookFragmentTest extends BaseActivityInstrumentationTestCase<MainMenuActivity> {
	...
	private static String FIRST_TEST_LOOK_NAME_PACKED;
	private static String SECOND_TEST_LOOK_NAME_PACKED;

	private static String FIRST_TEST_LOOK_NAME_UNPACKED;
	private static String SECOND_TEST_LOOK_NAME_UNPACKED;
	...
	public void setUp() throws Exception {
		...
		FIRST_TEST_LOOK_NAME_PACKED = FIRST_TEST_LOOK_NAME + "_" + projectManager.getCurrentProject().getName();
		SECOND_TEST_LOOK_NAME_PACKED = SECOND_TEST_LOOK_NAME + "_" + projectManager.getCurrentProject().getName();

		FIRST_TEST_LOOK_NAME_UNPACKED = FIRST_TEST_LOOK_NAME + "_" + projectManager.getCurrentProject().getName() + "_"
				+ solo.getString(R.string.unpacking_addition);
		SECOND_TEST_LOOK_NAME_UNPACKED = SECOND_TEST_LOOK_NAME + "_" + projectManager.getCurrentProject().getName()
	...
	public void testBackpackLookContext() {
		LookAdapter adapter = getLookAdapter();
		assertNotNull("Could not get Adapter", adapter);
		...
		assertTrue("BackPack title didn't show up",
				solo.waitForText(solo.getString(R.string.backpack_title), 0, TIME_TO_WAIT_BACKPACK));
		assertTrue("Look wasn't packed!", solo.waitForText(SECOND_TEST_LOOK_NAME_PACKED, 0, TIME_TO_WAIT));
	}

	public void testBackpackLookDoubleContext() {
		LookAdapter adapter = getLookAdapter();
		assertNotNull("Could not get Adapter", adapter);
		clickOnContextMenuItem(SECOND_TEST_LOOK_NAME, solo.getString(R.string.backpack));
		solo.waitForDialogToClose(TIME_TO_WAIT_BACKPACK);
		...
		assertTrue("BackPack title didn't show up",
				solo.waitForText(solo.getString(R.string.backpack_title), 0, TIME_TO_WAIT_BACKPACK));
		assertTrue("Look wasn't unpacked!", solo.waitForText(FIRST_TEST_LOOK_NAME_PACKED, 0, TIME_TO_WAIT));
		assertTrue("Look wasn't unpacked!", solo.waitForText(SECOND_TEST_LOOK_NAME_PACKED, 0, TIME_TO_WAIT));
	}
	...
	public void testSimpleUnpacking() {
		...
		clickOnContextMenuItem(FIRST_TEST_LOOK_NAME_PACKED, solo.getString(R.string.unpack));
		solo.waitForDialogToClose(TIME_TO_WAIT_BACKPACK);
		solo.goBack();

		assertTrue("Look wasn't unpacked!", solo.waitForText(FIRST_TEST_LOOK_NAME_UNPACKED, 0, TIME_TO_WAIT));
	}
	...
	public void testSimpleUnpackingAndDelete() {
		...
		solo.sleep(TIME_TO_WAIT_BACKPACK);
		solo.goBack();
		clickOnContextMenuItem(FIRST_TEST_LOOK_NAME, solo.getString(R.string.delete));
	...
	public void testMultipleUnpacking() {
		...
		clickOnContextMenuItem(SECOND_TEST_LOOK_NAME, solo.getString(R.string.backpack));
		solo.sleep(TIME_TO_WAIT_BACKPACK);
		clickOnContextMenuItem(SECOND_TEST_LOOK_NAME_PACKED, solo.getString(R.string.unpack));
		solo.waitForDialogToClose(TIME_TO_WAIT_BACKPACK);
		solo.goBack();

		assertTrue("Look wasn't unpacked!", solo.waitForText(SECOND_TEST_LOOK_NAME_UNPACKED, 0, TIME_TO_WAIT));
		int newCount = adapter.getCount();

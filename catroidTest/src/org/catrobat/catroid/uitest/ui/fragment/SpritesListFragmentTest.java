...
public class SpritesListFragmentTest extends BaseActivityInstrumentationTestCase<MainMenuActivity> {
	...
	public void testDeleteWithGroups() {
		...
		assertEquals("The wrong number of items is in the spritelist", 1, ProjectManager.getInstance()
				.getCurrentProject().getSpriteList().size());
	}
	...
	public void testGroupAddRenameAndDeleteActions() {
		...
		assertTrue("Group item has not been converted to SingleSprite", project.getSpriteList().get(1) instanceof SingleSprite);
		...
		assertEquals("Number of spriteList items is wrong", 4, project.getSpriteList().size());
	}
	...
	public void testLoadOldProjectAndConvertSpritesForGrouping() {
		...
		for (int spritePosition = 0; spritePosition < project.getSpriteList().size(); spritePosition++) {
			assertTrue("Group item has not been converted to SingleSprite",
					project.getSpriteList().get(spritePosition) instanceof SingleSprite);
		}
	}
	...
	public void testExpandAndCollapseGroup() {
		...
		checkGroupIndicatorVisibility(GONE, VISIBLE);
		assertFalse("Group is expanded", solo.searchText(groupItemNameOfFirstGroup, 0, false, true));
		solo.clickOnText(firstGroupName);

		checkGroupIndicatorVisibility(VISIBLE, GONE);
	...
	private void checkGroupIndicatorVisibility(int expandedIndicatorVisibility, int collapsedIndicatorActiveVisibility) {
		solo.sleep(200);
		assertTrue("expandedIndicator " + getAssertMessageAffix(expandedIndicatorVisibility),
				solo.getView(R.id.sprite_group_indicator_expanded, 1).getVisibility() == expandedIndicatorVisibility);
		assertTrue("collapsedIndicator " + getAssertMessageAffix(collapsedIndicatorActiveVisibility),
				solo.getView(R.id.sprite_group_indicator_collapsed, 1).getVisibility() == collapsedIndicatorActiveVisibility);
	}

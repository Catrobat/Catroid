...
public class FormulaEditorListFragmentTest extends BaseActivityInstrumentationTestCase<MainMenuActivity> {
	...
	private static final int[] FUNCTIONS_ITEMS = { R.string.formula_editor_function_sin,
			...
			R.string.formula_editor_function_length, R.string.formula_editor_function_letter, R.string.formula_editor_function_join,
			R.string.formula_editor_function_number_of_items, R.string.formula_editor_function_list_item, R.string.formula_editor_function_contains };
	...
	public void testFunctionsListElements() {
		...
		assertTrue(solo.waitForFragmentByTag(FormulaEditorFragment.FORMULA_EDITOR_FRAGMENT_TAG, 1000));

		solo.clickOnText(solo.getString(R.string.formula_editor_math));
		assertTrue(solo.waitForFragmentByTag(FormulaEditorListFragment.FUNCTION_TAG, 1000));

		int numberOfFunctions = FUNCTIONS_ITEMS.length;

		for (int i = 0; i < numberOfFunctions; i++) {

			String listItemText = solo.getString(FUNCTIONS_ITEMS[i]) + solo.getString
					(FUNCTIONS_PARAMETERS[i]);
			assertTrue(solo.waitForText(Pattern.quote(listItemText), 1, 2000));
		}
	}
}

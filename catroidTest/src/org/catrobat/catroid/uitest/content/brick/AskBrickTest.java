...
import org.catrobat.catroid.formulaeditor.SensorManager;
...
public class AskBrickTest extends ActivityInstrumentationTestCase2<StageActivity> {
	...
	public void testCancelAndResetAskAnswer() {
		...
		//SensorHandler.startSensorListener(instrument.getTargetContext());
		//UtilSpeechRecognition.getInstance().setAnswerResult(mockRecognizedWords, Activity.RESULT_OK);
		//assertEquals("Mocked Sensor value was not saved!","mock",SensorHandler.getSensorValue(Sensors.ANSWER));
		...
		assertEquals("Recognize intent wasn't fired as expected, monitor hit", 1, recognizeMonitor2.getHits());

...
public class BroadcastLandscapeTest extends BaseActivityInstrumentationTestCase<MainMenuActivity> {
	...
	@Device
	public void testBroadcastWithLandscapeMode(){
		UiTestUtils.clickOnBottomBar(solo, R.id.button_play);
		solo.waitForText("1.0");
	}
	...
	private void createProject() {
		project = new Project(null, UiTestUtils.DEFAULT_TEST_PROJECT_NAME, true);

		sprite = new Sprite("cat");

		Script script_broadcast = new BroadcastScript(defaultBroadcastMessage);
		Script script_start = new StartScript();
		DataContainer dataContainer = project.getDataContainer();
		dataContainer.addProjectUserVariable("variable");

		ShowTextBrick showTextBrick = new ShowTextBrick(50,50);
		showTextBrick.setUserVariable(dataContainer.getUserVariable("variable",sprite));
		ChangeVariableBrick changeVariableBrick = new ChangeVariableBrick(1);
		changeVariableBrick.setUserVariable(dataContainer.getUserVariable("variable",sprite));
		WaitBrick waitBrick = new WaitBrick(1000);
		BroadcastBrick broadcastBrick = new BroadcastBrick(defaultBroadcastMessage);

		script_start.addBrick(waitBrick);
		script_start.addBrick(broadcastBrick);

		script_broadcast.addBrick(showTextBrick);
		script_broadcast.addBrick(changeVariableBrick);

		sprite.addScript(script_start);
		sprite.addScript(script_broadcast);
		project.addSprite(sprite);

		ProjectManager.getInstance().setProject(project);
		ProjectManager.getInstance().setCurrentSprite(sprite);
		ProjectManager.getInstance().setCurrentScript(script_start);
	}
}

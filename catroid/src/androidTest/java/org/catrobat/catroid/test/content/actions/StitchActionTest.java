...
public class StitchActionTest {
	...
	public void setUp() throws Exception {
		...
		Mockito.when(StageActivity.stageListener.getEmbroideryManager()).thenReturn(Mockito.mock(EmbroideryManager.class));
	}

	@Test
	public void testAddSingleStitchPoint() {
		testSprite1.getActionFactory().createStitchAction(testSprite1).act(1f);
		Mockito.verify(StageActivity.stageListener.getEmbroideryManager(), Mockito.times(1)).addStitchPoint(spriteCoords1);
	}
	...
	public void testAddPointsTwoSprites() {
		...
		Mockito.verify(StageActivity.stageListener.getEmbroideryManager(), Mockito.times(1)).addStitchPoint(spriteCoords1);
		Mockito.verify(StageActivity.stageListener.getEmbroideryManager(), Mockito.times(1)).addStitchPoint(spriteCoords2);
	}
}

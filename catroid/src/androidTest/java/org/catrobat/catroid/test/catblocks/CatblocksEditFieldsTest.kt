    ...
    private fun editBrickFormulas(bricksToEdit: List<FormulaBrick>) {
        ...
        for (brickToEdit in bricksToEdit) {
            for (fieldID in brickToEdit.formulaMap.keys) {
                testFormulaField(brickToEdit, fieldID, webViewUtils)
            }
        }
    }

    private fun testFormulaField(
        brickToEdit: FormulaBrick,
        fieldID: Brick.FormulaField,
        ...
        webViewUtils.waitForElement(catblocksFieldID)
        scrollToField(catblocksFieldID)

        webViewUtils.clickElement(catblocksFieldID)

        Thread.sleep(200)

        val numberToEnter = enterRandomNumberFormulaEditor()
        ...
        Assert.assertEquals("$numberToEnter", catroidFieldContent)

        val textInCatblocksField = webViewUtils.getText(
            "$catblocksFieldID .blocklyText")?.trim()?.trim('"')?.trim()

        Assert.assertEquals("$numberToEnter", textInCatblocksField)
    }

    private fun enterRandomNumberFormulaEditor(): Int {

...
import org.catrobat.catroid.test.content.bricks.CloneThreePartCompositeBrickUpdateDataTest
...
class CloneThreePartCompositeBrickUpdateDataTest {
    @JvmField
    ...
    @Test
    fun testUpdateUserVariableInNestedBricks() {
    ...
    @Test
    fun testUpdateUserListInNestedBricks() {
    ...
    @Test
    fun testUpdateUserVariableInSecondaryNestedBricks() {
        sprite!!.addUserVariable(USER_VARIABLE)
        val variableBrick: UserVariableBrickWithFormula =
            SetVariableBrick(Formula(0), USER_VARIABLE)
        compositeBrick!!.secondaryNestedBricks.add(variableBrick)
        val cloneSprite = SpriteController().copyForCloneBrick(sprite)
        val clonedCompositeBrick = cloneSprite.getScript(0).getBrick(0) as CompositeBrick
        val clonedUserVariableBrick =
            clonedCompositeBrick.secondaryNestedBricks[0] as UserVariableBrickWithFormula
        Assert.assertEquals(USER_VARIABLE, clonedUserVariableBrick.userVariable)
        Assert.assertNotSame(USER_VARIABLE, clonedUserVariableBrick.userVariable)
    }

    @Test
    fun testUpdateUserListInSecondaryNestedBricks() {
        sprite!!.addUserList(USER_LIST)
        val listBrick: UserListBrick = AddItemToUserListBrick(0.0)
        listBrick.userList = USER_LIST
        compositeBrick!!.secondaryNestedBricks.add(listBrick)
        val cloneSprite = SpriteController().copyForCloneBrick(sprite)
        val clonedCompositeBrick = cloneSprite.getScript(0).getBrick(0) as CompositeBrick
        val clonedUserVariableBrick = clonedCompositeBrick.secondaryNestedBricks[0] as UserListBrick
        Assert.assertEquals(USER_LIST, clonedUserVariableBrick.userList)
        Assert.assertNotSame(USER_LIST, clonedUserVariableBrick.userList)
    }
    ...
    companion object {
        ...
        fun data(): Iterable<Array<Any>> {
            return Arrays.asList(
                *arrayOf(
                    arrayOf(
                        IfLogicBeginBrick::class.java.simpleName, IfLogicBeginBrick::class.java
                    ), arrayOf(
                        PhiroIfLogicBeginBrick::class.java.simpleName,
                        PhiroIfLogicBeginBrick::class.java
                    ), arrayOf(
                        RaspiIfLogicBeginBrick::class.java.simpleName,

...
Program 'test x' {
    ...
    Background {
      ...
      Scripts {
        ...
        When Raspberry Pi pin changes to (pin: (3), position: (high)) {
          ...
          For each tuple of items in selected lists stored in variables with the same name, assert value equals to the expected item of reference list (lists: (*List*), value: (0), reference list: (*List*)) {
          }
          Wait until all other scripts have stopped;
          Single tap at (x: (- 100), y: (- 200));
          Touch at position for seconds (x: (- 100), y: (- 200), seconds: (0.3));
  ...
  Scene 'Scene 2' {
    Background {
      Scripts {
        When you receive (message: ('message 1')) {
          `label1 [input 1] label2 [input 2]` ([input 1]: ('hallo'), [input 2]: ('world'));
        }
      }
      User Defined Bricks {
        Define (user defined brick: (`label1 [input 1] label2 [input 2]`), screen refresh: (on)) {
          Broadcast (message: ('message 1'));
          Broadcast and wait (message: ('message 1'));
        }
      }
    }
  }
...
//      Scripts {
//        When you receive (message: ('message 1')) {
//          `label1 [input 1] label2 [input 2]` ([input 1]: ('asdf'), [input 2]: ('jklö'));
//        }
//      }
//      User Defined Bricks {
//        Define (user defined brick: (`label1 [input 1] label2 [input 2]`), screen refresh: (off)) {
//          `label1 [input 1] label2 [input 2]` ([input 1]: ('hallo'), [input 2]: ('world'));
//          Broadcast (message: ('message 1'));
//          Broadcast and wait (message: ('message 1'));
//        }

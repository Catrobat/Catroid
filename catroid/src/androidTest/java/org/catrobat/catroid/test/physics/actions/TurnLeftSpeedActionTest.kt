...
import org.catrobat.catroid.test.physics.actions.TurnLeftSpeedActionTest
...
class TurnLeftSpeedActionTest {
    ...
    private fun initLeftSpeedValue(speed: Float) {
        val physicsObject = physicsWorld!!.getPhysicsObject(sprite)
        val action = sprite!!.actionFactory.createTurnLeftSpeedAction(
            sprite,
            SequenceAction(),
            Formula(speed)
        )
    ...
    fun testBrickWithStringFormula() {
        val physicsObject = physicsWorld!!.getPhysicsObject(sprite)
        sprite!!.actionFactory.createTurnLeftSpeedAction(
            sprite,
            SequenceAction(),
    ...
    fun testNullFormula() {
        val physicsObject = physicsWorld!!.getPhysicsObject(sprite)
        sprite!!.actionFactory.createTurnLeftSpeedAction(sprite, SequenceAction(), null).act(1.0f)

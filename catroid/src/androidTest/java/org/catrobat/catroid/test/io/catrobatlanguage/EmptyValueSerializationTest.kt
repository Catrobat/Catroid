...
class EmptyValueSerializationTest(
    ...
    companion object {
     ...
     /*
                ...
                arrayOf(
                    TapForBrick(),
                    "Touch at position for seconds (x: (0), y: (0), seconds: (0));\n"
                ),
                arrayOf(
                    TouchAndSlideBrick(),
                    "Touch at position and slide to position in seconds (start x: (0), start y: " +
                        "(0), to x: (0), to y: (0), seconds: (0));\n"
                ),
                // TODO: should be empty
                arrayOf(OpenUrlBrick(), "Open in browser (url: (0));\n"),
                arrayOf(PlaceAtBrick(), "Place at (x: (0), y: (0));\n"),
                arrayOf(SetXBrick(), "Set (x: (0));\n"),
                arrayOf(SetYBrick(), "Set (y: (0));\n"),
                arrayOf(ChangeXByNBrick(0), "Change x by (value: (0));\n"),
                arrayOf(ChangeYByNBrick(0), "Change y by (value: (0));\n"),
//                arrayOf(GoToBrick(), "Go to (target: ());\n"),
                arrayOf(IfOnEdgeBounceBrick(), "If on edge, bounce;\n"),
                arrayOf(MoveNStepsBrick(), "Move (steps: (0));\n"),
                arrayOf(TurnLeftBrick(), "Turn (direction: ('left'), degrees: (0));\n"),
                arrayOf(TurnRightBrick(), "Turn (direction: ('right'), degrees: (0));\n"),
                arrayOf(PointInDirectionBrick(), "Point in direction (degrees: (0));\n"),
                arrayOf(PointToBrick(), "Point towards (actor or object: ());\n"),
//                arrayOf(SetRotationStyleBrick(), "Set (rotation style: ());\n"),
                arrayOf(GlideToBrick(), "Glide to (x: (0), y: (0), seconds: (0));\n"),
                arrayOf(GoNStepsBackBrick(), "Go back (number of layers: (0));\n"),
                arrayOf(ComeToFrontBrick(), "Come to front;\n"),
                arrayOf(
                    SetCameraFocusPointBrick(),
                    "Become focus point with flexibility in percent (horizontal: (0), vertical: (0));"
                ),
                arrayOf(VibrationBrick(), "Vibrate for (seconds: (0));"),
                arrayOf(SetPhysicsObjectTypeBrick(), "Set (motion type: ());\n"),
                arrayOf(
                    SetVelocityBrick(),
                    "Set velocity to (x steps/second: (0), y steps/second: (0));\n"
                ),
                arrayOf(TurnLeftBrick(), "Spin (direction: (left), degrees/second: (0));\n"),
                arrayOf(TurnRightBrick(), "Spin (direction: (right), degrees/second: (0));\n"),
                arrayOf(
                    SetGravityBrick(),
                    "Set gravity for all actors and objects to (x steps/second²: (0), y " +
                        "steps/second²: (0));\n"
                ),
                arrayOf(SetMassBrick(), "Set mass to (kilograms: (0));\n"),
                arrayOf(SetBounceBrick(), "Set bounce factor to (percentage: (0));\n"),
                arrayOf(SetFrictionBrick(), "Set friction to (percentage: (0));\n"),
                arrayOf(FadeParticleEffectBrick(), "Fade particle (effect: ());\n"),
                arrayOf(PlaySoundBrick(), "Start (sound: ());\n"),
                arrayOf(
                    PlaySoundAndWaitBrick(),
                    "Start sound and skip seconds (sound: (), seconds: (0));\n"
                ),
                arrayOf(
                    PlaySoundAtBrick(),
                    "Start sound and skip seconds (sound: (), seconds: (0));\n"
                ),
                arrayOf(StopSoundBrick(), "Stop (sound: ());\n"),
                arrayOf(StopAllSoundsBrick(), "Stop all sounds;\n"),
                arrayOf(SetVolumeToBrick(), "Set (volume percentage: (0));\n"),
                arrayOf(ChangeVolumeByNBrick(), "Change volume by (value: (0));\n"),
                arrayOf(SetInstrumentBrick(), "Set (instrument: ());\n"),
                arrayOf(PlayNoteForBeatsBrick(), "Play (note: (), number of beats: (0));\n"),
                arrayOf(PlayDrumForBeatsBrick(), "Play (drum: (), number of beats: (0));\n"),
                arrayOf(SetTempoBrick(), "Set (tempo: (0));\n"),
                arrayOf(ChangeTempoByNBrick(), "Change tempo by (value: (0));\n"),

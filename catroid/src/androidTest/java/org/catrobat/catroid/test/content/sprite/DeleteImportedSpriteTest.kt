...
class DeleteImportedSpriteTest {
    ...
    private fun importSprite() {
        ...
        Espresso.onView(withId(R.id.dialog_new_look_from_local)).perform(click())
        Espresso.onView(withText(R.string.import_from_project)).check(matches(isDisplayed()))

        Espresso.onView(withText(localProject.name)).perform(click())
        Espresso.onView(withText(R.string.import_object)).check(matches(isDisplayed()))

        Espresso.onView(withText(localProject.defaultScene.spriteList[1].name)).perform(click())
    ...
    private fun createProjects(projectName: String) {
        project = Project(context, projectName)

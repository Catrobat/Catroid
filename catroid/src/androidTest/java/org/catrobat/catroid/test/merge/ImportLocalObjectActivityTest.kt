...
class ImportLocalObjectActivityTest {
    ...
    fun setUp() {
        ...
        ImportLocalObjectActivity.sceneToImportFrom =
            multipleScenesWithMultipleSpritesProject.defaultScene
        ImportLocalObjectActivity.spriteToImport =
            multipleScenesWithMultipleSpritesProject.defaultScene.backgroundSprite
    }

    private fun initProjects() {
        baseProject = DefaultProjectHandler
            .createAndSaveDefaultProject("base", context, false)
        multipleScenesWithMultipleSpritesProject = DefaultProjectHandler
        ...
        multipleScenesWithMultipleSpritesProject.addScene(scene1)
        multipleScenesWithMultipleSpritesProject.getSceneByName(scene1.name)?.spriteList?.add(
            baseProject.defaultScene?.getSprite(
                context.getString(R.string.default_project_cloud_sprite_name_1)
            )
        )
        ...
        projectManager.loadProject(File(FlavoredConstants.DEFAULT_ROOT_DIRECTORY, "base"))
    }
    ...
    fun testImportSpriteView() {
        ...
        Espresso.onView(withText(R.string.import_object))
        ...
        Espresso.onView(
            withText(
                baseActivityTestRule.activity.applicationContext
                    .getString(R.string.fragment_sprite_text_description)
            )
        ).check(doesNotExist())

        multipleScenesWithMultipleSpritesProject.defaultScene.spriteList.forEach { sprite ->
            Espresso.onView(withText(sprite.name)).check(matches(isDisplayed()))
        }
    }
    ...
    fun testImportFromSceneView() {
        ...
        Espresso.onView(withText(R.string.import_from_scene))
        Espresso.onView(withId(R.id.bottom_bar)).check(
            matches(ViewMatchers.withEffectiveVisibility(ViewMatchers.Visibility.GONE))
        )

        multipleScenesWithMultipleSpritesProject.sceneList.forEach { scene ->
            Espresso.onView(withText(scene.name)).check(
                matches(isDisplayed())
            )
        }
    }
    ...
    fun testImportFromProjectView() {
        ...
        Espresso.onView(withText(R.string.import_from_project))
        Espresso.onView(withId(R.id.bottom_bar)).check(
            matches(ViewMatchers.withEffectiveVisibility(ViewMatchers.Visibility.GONE))
        )

        Espresso.onView(withText(multipleScenesWithMultipleSpritesProject.name)).check(
            matches(isDisplayed())
        )

        Espresso.onView(withText(baseProject.name)).check(
            matches(isDisplayed())
        )
    }

...
class MergeTestUtils {
    fun assertSuccessfulSpriteImport(
        ...
        Assert.assertFalse(TestUtils.checkForDuplicates(currentProject.userLists.map { it.name }))
        Assert.assertFalse(TestUtils.checkForDuplicates(currentProject.userVariables.map { it.name }))
        Assert.assertFalse(
            TestUtils.checkForDuplicates(

...
class CompositeMiddleBrickMoveTest {
    private lateinit var project: Project
    private val oneBlockDown: Float = -310f
    private val oneBlockUp: Float = 360f
    private lateinit var ifBrick: IfLogicBeginBrick
    ...
    fun testMoveEndBrick() {
        ...
        Assert.assertEquals(ifBrick.secondaryNestedBricks.size, 1)

        onView(
            withId(R.id.brick_if_else_label)
        ).perform(
        ...
        Assert.assertEquals(ifBrick.secondaryNestedBricks.size, 0)
        onView(
            withId(R.id.brick_if_else_label)
        ).perform(
        ...
        Assert.assertEquals(ifBrick.secondaryNestedBricks.size, 2)
    ...
    private fun createProject(projectName: String) {
        ...
        repeatBrick.addBrick(ChangeSizeByNBrick(3.0))
        ifBrick.addBrickToIfBranch(changeSizeBrick)
        ifBrick.addBrickToElseBranch(repeatBrick)

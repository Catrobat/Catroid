...
class FormulaEditorDeleteListTest {
    ...
    @Before
    fun setUp() {
        script = UiTestUtils.createProjectAndGetStartScript("FormulaEditorDeleteListTest")
    ...
    @Test
    fun deleteLocalListTest() {
        val itemName = "item"
        onDataList().performAdd(itemName, ItemType.LIST, ItemScope.LOCAL)
        onDataList().onListAtPosition(0).performDelete()
        onRecyclerView().checkHasNumberOfItems(0)
    }

    @Category(AppUi::class, Smoke::class)
    @Test
    fun deleteGlobalListTest() {
        val itemName = "item"
        onDataList().performAdd(itemName, ItemType.LIST, ItemScope.GLOBAL)
        onDataList().onListAtPosition(0).performDelete()
        onRecyclerView().checkHasNumberOfItems(0)
    }
    ...
    @Category(AppUi::class, Smoke::class)
    @Test
    fun deleteLocalListWhenInUseTest() {
        val itemName = "item"
        onDataList().performAdd(itemName, ItemType.LIST, ItemScope.LOCAL)

        onDataList().onListAtPosition(0).performSelect()
    ...
    @Category(AppUi::class, Smoke::class)
    @Test
    fun deleteGlobalListWhenInUseTest() {
        val itemName = "item"
        onDataList().performAdd(itemName, ItemType.LIST, ItemScope.GLOBAL)
    ...
    @Category(AppUi::class, Smoke::class)
    @Test
    fun deleteLocalListWhenInDropDownTest() {
        script!!.addBrick(AddItemToUserListBrick(0.0))
        val itemName = "item"
        onDataList().performAdd(itemName, ItemType.LIST, ItemScope.LOCAL)

        onView(isRoot()).perform(pressBack())
        onView(isRoot()).perform(pressBack())
        onBrickAtPosition(2).onFormulaTextField(R.id.brick_set_x_edit_text)
            .perform(click())
        onFormulaEditor().performOpenDataFragment()
        onDataList().onListAtPosition(0).performDelete()
    ...
    @Category(AppUi::class, Smoke::class)
    @Test
    fun deleteGlobalListWhenInDropDownTest() {
        script!!.addBrick(AddItemToUserListBrick(0.0))
        val itemName = "item"
        onDataList().performAdd(itemName, ItemType.LIST, ItemScope.GLOBAL)

...
import static org.catrobat.catroid.uitest.util.UiTestUtils.DRAG_FRAMES;
...
public class UserBrickTest extends BaseActivityInstrumentationTestCase<MainMenuActivity> {
	...
	public void setUp() throws Exception {
		...
		Project project = UiTestUtils.createTestProjectWithUserBrick();

		solo.sleep(2000);
		UiTestUtils.getIntoScriptActivityFromMainMenu(solo, project);
	}
	...
	public void testEditUserBricksInEditorAndScript() {
		...
		solo.sleep(200);
		solo.waitForDialogToOpen();
		String proposedUserBrickName = solo.getString(R.string.new_user_brick) + " 2";
	...
	public void testEqualUserBrickNames() {
		solo.clickOnView(solo.getCurrentActivity().findViewById(R.id.button_add));
		UiTestUtils.clickOnBrickCategory(solo, solo.getCurrentActivity().getString(R.string.category_user_bricks));
		solo.waitForText(UiTestUtils.TEST_USER_BRICK_NAME);
		...
		solo.clickOnView(solo.getCurrentActivity().findViewById(R.id.button_add));
		solo.waitForDialogToOpen();
		assertTrue("Wrong name proposed", solo.waitForText(UiTestUtils.SECOND_TEST_USER_BRICK_NAME, 1, 1000, false, true));

		solo.clearEditText(2);
		solo.enterText(2, UiTestUtils.TEST_USER_BRICK_NAME);
		solo.sleep(500);
		assertTrue("No name given message shown",
				solo.searchText(solo.getString(R.string.user_brick_name_given), 0, false, true));
		assertFalse("Button not disabled", solo.getButton(1).isEnabled());
		...
		assertTrue("No name given message shown",
				solo.searchText(solo.getString(R.string.user_brick_name_given), 1, false, true));
		assertFalse("Button not disabled", solo.getButton(2).isEnabled());
	}
	...
	public void testChangeDeleteUserBrickElements() {
		...
		//delete all elements but first (= function name)
		solo.clickOnButton(0);
		solo.sleep(300);
		solo.clickOnButton(0);
		solo.sleep(300);

		assertEquals("UserBrick Function name shouldn't have a delete button", 0,
				RobotiumUtils.removeInvisibleViews(solo.getCurrentViews(Button.class)).size());

		assertTrue("Function name should not have disappeared", solo.waitForText(newTextName, 0, 1000, false, true));
		assertFalse("Variable and LineBreak should have disappeared",
	...
	public void testBackPackUserBricks() {
		UiTestUtils.getIntoUserBrickOverView(solo);

		backPackFirstUserBrickViaActionMode(DEFAULT_USERBRICK_GROUP_NAME);
	...
	public void testBackPackUserBricksDouble() {
		UiTestUtils.getIntoUserBrickOverView(solo);

		backPackFirstUserBrickViaActionMode(DEFAULT_USERBRICK_GROUP_NAME);
		solo.goBack();
		backPackFirstUserBrickViaActionMode(SECOND_USERBRICK_GROUP_NAME);
	...
	public void testBackPackUserBricksSimpleUnpacking() {
		UiTestUtils.getIntoUserBrickOverView(solo);
	...
	public void testBackPackUserBricksDelete() {
		UiTestUtils.getIntoUserBrickOverView(solo);

		backPackFirstUserBrickViaActionMode(DEFAULT_USERBRICK_GROUP_NAME);

		BackPackUserBrickListAdapter adapter = getBackPackUserBrickAdapter();
		int oldCount = adapter.getCount();

		deleteFirstUserBrickViaActionMode();
	...
	public void testBackPackUserBricksDeleteAll() {
		UiTestUtils.getIntoUserBrickOverView(solo);
	...
	public void testBackPackAllUserBricksDifferentProgrammes() {
		UiTestUtils.getIntoUserBrickOverView(solo);
		addOneUserBrickInPrototypeView(UiTestUtils.SECOND_TEST_USER_BRICK_NAME);
	...
	public void testBackPackUserBrickGroupWithSameName() {
		...
		assertTrue("No userbrick group already existing warning appeared!",
				solo.searchText(solo.getString(R.string.group_or_object_name_given), 0, false, true));
	}
	...
	private BackPackUserBrickListFragment getBackPackUserBrickFragment() {
		BackPackActivity activity = (BackPackActivity) solo.getCurrentActivity();
		return (BackPackUserBrickListFragment) activity.getFragment(BackPackActivity.FRAGMENT_BACKPACK_USERBRICKS);
	}

	private BackPackUserBrickListAdapter getBackPackUserBrickAdapter() {
		return (BackPackUserBrickListAdapter) getBackPackUserBrickFragment().getListAdapter();
	}

	private void deleteFirstUserBrickViaActionMode() {
		solo.sleep(200);
		UiTestUtils.openActionMode(solo, solo.getString(R.string.delete), R.id.delete);
		solo.sleep(500);
	...
	private void backPackFirstUserBrickViaActionMode(String userBrickGroupName) {
		solo.sleep(200);
		UiTestUtils.openActionMode(solo, solo.getString(R.string.backpack), R.id.backpack);
		solo.sleep(500);
		if (solo.searchText(solo.getString(R.string.packing), 0, false, true)) {
	...
	private void checkVisibilityOfViews(int imageVisibility, int userBrickGroupNameVisibility, int userBrickGroupDetailsVisibility,
		...
		assertTrue("Userbrick group image " + getAssertMessageAffix(imageVisibility),
				solo.getView(R.id.list_item_image_view).getVisibility() == imageVisibility);
		assertTrue("Userbrick group name " + getAssertMessageAffix(userBrickGroupNameVisibility),
				solo.getView(R.id.list_item_text_view).getVisibility() == userBrickGroupNameVisibility);
		assertTrue("Userbrick group details " + getAssertMessageAffix(userBrickGroupDetailsVisibility),
				solo.getView(R.id.list_item_details).getVisibility() == userBrickGroupDetailsVisibility);
		assertTrue("Checkboxes " + getAssertMessageAffix(checkBoxVisibility),
				solo.getView(R.id.list_item_checkbox).getVisibility() == checkBoxVisibility);
	}

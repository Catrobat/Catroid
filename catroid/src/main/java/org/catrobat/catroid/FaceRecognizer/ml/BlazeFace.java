...
public class BlazeFace {
    ...
    private class Detection {
        ...
        Detection(RectF location, float score, float[] keypoints) {
            this.location = location;
            this.score = score;
            this.keypoints = keypoints;
        }
    ...
    public static BlazeFace create(
        ...
        try {
            ...
            b.interpreter = new Interpreter(loadModelFile(assetManager), options);
        } catch (Exception e) {
    ...
    public List<RectF> detect(Bitmap bitmap) {
        ...
        for (int i = 0; i < NUM_BOXES; i++) {
            ...
            for (int k = 0; k < 6; k++) {
                float kx = outputBoxes.get(i * NUM_COORDS + 4 + k * 2);
                float ky = outputBoxes.get(i * NUM_COORDS + 5 + k * 2);
                keypoints[k * 2] =
                        (kx / X_SCALE * anchors.get(i).w + anchors.get(i).x_center)
                                * INPUT_SIZE_WIDTH;
                keypoints[k * 2 + 1] =
                        (ky / Y_SCALE * anchors.get(i).h + anchors.get(i).y_center)
                                * INPUT_SIZE_HEIGHT;
            }

            detections.add(new Detection(new RectF(xmin, ymin, xmax, ymax), score, keypoints));
        }
        ...
        List<FaceBox> retained = WeightedNonMaxSuppression(indexed_scores, detections);

        Trace.endSection(); // "detect"

        List<RectF> boxes = new ArrayList<>();
        for (FaceBox f : retained) {
    ...
    private List<FaceBox> lastFaces = new ArrayList<>();

    /** Runs detection and returns the boxes together with their landmarks. */
    @RequiresApi(api = Build.VERSION_CODES.N)
    public List<FaceBox> detectWithLandmarks(Bitmap bitmap) {
        detect(bitmap);
        return lastFaces;
    }

    private List<FaceBox> WeightedNonMaxSuppression(List<IndexedScore> indexed_scores,
        ...
        List<FaceBox> output_locations = new ArrayList<>();
        ...
        while (!remained_indexed_scores.isEmpty()) {
            ...
            output_locations.add(new FaceBox(weighted_location, detection.keypoints));
        }

...
import org.catrobat.catroid.databinding.ActivitySpriteBinding
import org.catrobat.catroid.databinding.DialogNewLookBinding
import org.catrobat.catroid.databinding.DialogNewSoundBinding
import org.catrobat.catroid.databinding.DialogNewUserDataBinding
...
@Suppress("LargeClass")
open class SpriteActivity : BaseActivity() {

    companion object {
        private val TAG = SpriteActivity::class.java.simpleName
        private const val IMPORT_FILE_TYPE = "image/*"
    ...
    private lateinit var binding: ActivitySpriteBinding
    private lateinit var dialogNewLookBinding: DialogNewLookBinding
    private lateinit var dialogNewSoundBinding: DialogNewSoundBinding
    private lateinit var dialogNewUserDataBinding: DialogNewUserDataBinding
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivitySpriteBinding.inflate(layoutInflater)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        val isDragAndDropActiveInFragment =
            currentFragment is ScriptFragment && (currentFragment as ScriptFragment?)?.isCurrentlyMoving == true
    ...
    public override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == TestResult.STAGE_ACTIVITY_TEST_SUCCESS || resultCode == TestResult.STAGE_ACTIVITY_TEST_FAIL) {
            ...
            val testResult = ClipData.newPlainText(
                "TestResult",
                projectManager.currentProject.name + "" + message
            )
            clipboard.setPrimaryClip(testResult)
        }

        if (resultCode != RESULT_OK) {
            if (SettingsFragment.isCastSharedPreferenceEnabled(this) && projectManager.currentProject.isCastProject && !CastManager.getInstance().isConnected
            ) {
                CastManager.getInstance().openDeviceSelectorOrDisconnectDialog(this)
            }
            return
        }
        executeOnRequestCode(requestCode, data)
    }

    @Suppress("ComplexMethod")
    private fun executeOnRequestCode(requestCode: Int, data: Intent?) {
        val uri: Uri?
        when (requestCode) {
            SPRITE_POCKET_PAINT -> {
            ...
            REQUEST_CODE_VISUAL_PLACEMENT -> {
                ...
                updateFromVisualPlacement(extras)
            }
        }
    }

    private fun updateFromVisualPlacement(extras: Bundle) {
        val xCoordinate =
    ...
    fun handleAddSpriteButton() {
        dialogNewLookBinding = DialogNewLookBinding.inflate(layoutInflater)
        val alertDialog = AlertDialog.Builder(this)
            .setTitle(R.string.new_sprite_dialog_title)
            .setView(dialogNewLookBinding.root)
            .create()

        dialogNewLookBinding.dialogNewLookPaintroid.setOnClickListener {
            ImportFromPocketPaintLauncher(this)
                .startActivityForResult(SPRITE_POCKET_PAINT)
            alertDialog.dismiss()
        }

        dialogNewLookBinding.dialogNewLookMediaLibrary.setOnClickListener {
            ImportFormMediaLibraryLauncher(this, FlavoredConstants.LIBRARY_LOOKS_URL)
                .startActivityForResult(SPRITE_LIBRARY)
            alertDialog.dismiss()
        }

        dialogNewLookBinding.dialogNewLookGallery.setOnClickListener {
        ...
        dialogNewLookBinding.dialogNewLookCamera.setOnClickListener {
    ...
    fun handleAddSoundButton() {
        dialogNewSoundBinding = DialogNewSoundBinding.inflate(layoutInflater)
        val alertDialog = AlertDialog.Builder(this)
            .setTitle(R.string.new_sound_dialog_title)
            .setView(dialogNewSoundBinding.root)
            .create()

        dialogNewSoundBinding.dialogNewSoundRecorder.setOnClickListener {
            startActivityForResult(Intent(this, SoundRecorderActivity::class.java), SOUND_RECORD)
            alertDialog.dismiss()
        }

        dialogNewSoundBinding.dialogNewSoundMediaLibrary.setOnClickListener {
            ImportFormMediaLibraryLauncher(this, FlavoredConstants.LIBRARY_SOUNDS_URL)
                .startActivityForResult(SOUND_LIBRARY)
            alertDialog.dismiss()
        }

        dialogNewSoundBinding.dialogNewSoundGallery.setOnClickListener {
        ...
        if (BuildConfig.FEATURE_POCKETMUSIC_ENABLED) {
            dialogNewSoundBinding.dialogNewSoundPocketmusic.visibility = View.VISIBLE
            dialogNewSoundBinding.dialogNewSoundPocketmusic.setOnClickListener {
    ...
    private fun handleAddUserDataButton() {
        dialogNewUserDataBinding = DialogNewUserDataBinding.inflate(layoutInflater)
        dialogNewUserDataBinding.makeList.visibility = View.VISIBLE

        if (SettingsFragment.isMultiplayerVariablesPreferenceEnabled(applicationContext)) {
            dialogNewUserDataBinding.multiplayer.visibility = View.VISIBLE
            dialogNewUserDataBinding.multiplayer.setOnCheckedChangeListener { _, isChecked: Boolean ->
                dialogNewUserDataBinding.makeList.isEnabled = !isChecked
            }
        }
        ...
        builder.setTextWatcher(textWatcher)
            ...
            .setPositiveButton(
                ...
                addListAndVariableData(
                    textInput,
                    dialogNewUserDataBinding.global,
                    dialogNewUserDataBinding.multiplayer,
                    dialogNewUserDataBinding.makeList
                )
            }
        val alertDialog = builder.setTitle(R.string.formula_editor_variable_dialog_title)
            .setView(dialogNewUserDataBinding.root)
            .setNegativeButton(getString(R.string.cancel), null)
            .create()

        dialogNewUserDataBinding.makeList.setOnCheckedChangeListener { _, checked: Boolean ->
            ...
            dialogNewUserDataBinding.multiplayer.isEnabled = !checked
        }
        alertDialog.show()
    }

    private fun addListAndVariableData(
        textInput: String?,
        addToProjectUserDataRadioButton: RadioButton,
        multiplayerRadioButton: RadioButton,
        makeListCheckBox: CheckBox
    ) {
    ...
    fun handleAddUserListButton() {
        ...
        val builder = TextInputDialog.Builder(this)
            ...
            .setPositiveButton(
                ...
                val addToProjectUserData = dialogNewUserDataBinding.global.isChecked
                val userList = UserList(textInput)
                if (addToProjectUserData) {
                    currentProject.addUserList(userList)
                } else {

...
public final class LookController {
	...
	public static String getLookNameFromLookFile(File lookFile) {
		return lookFile.getName().substring(33, lookFile.getName().lastIndexOf("."));
	}
	...
	public static boolean loadFromPocketPaint(Intent intent, LookData lookData) {
		...
		FileChecksumContainer checksumContainer = projectManager.getFileChecksumContainer();

		File newLookFile = StorageHandler.copyFile(pathOfPocketPaintImage,
				projectManager.getCurrentScene().getSceneImageDirectoryPath(), checksumContainer);

		StorageHandler.deleteFile(lookData.getAbsolutePath(), checksumContainer);
...
//		}
	...
	public static LookData backpackHidden(LookData lookData) {
		String newLookFileName = getLookFileNameForBackpack(lookData);
		List<LookData> scope = BackPackListManager.getInstance().getAllBackPackedLooks();
		LookData backpackLookData = new LookData(Utils.getUniqueLookName(lookData.getName(), scope), newLookFileName);
		backpackLookData.isBackpackLookData = true;
	...
	public static LookData unpackVisible(LookData lookData) {
		...
		LookData lookDataToUnpack = new LookData(Utils.getUniqueLookName(lookData.getName(), scope), newLookFileName);

...
public final class OldSoundController {
	...
	private static final OldSoundController INSTANCE = new OldSoundController();
	private static final String TAG = OldSoundController.class.getSimpleName();

	private OnBackpackSoundCompleteListener onBackpackSoundCompleteListener;

	private OldSoundController() {
	}

	public static OldSoundController getInstance() {
		return INSTANCE;
	}
...
//		updateSoundAdapter(soundInfo, adapter, newSoundInfoTitle, false, false);
	...
	private void deleteSound(int position, List<SoundInfo> soundInfoList, Activity activity) {
		...
		if (soundInfoToDelete.isBackpackSoundInfo() && !otherSoundInfoItemsHaveAFileReference(soundInfoToDelete)) {
			StorageHandler.deleteFile(soundInfoList.get(position).getAbsolutePath());
		}

...
class ScriptController {
    ...
    private fun checkForUserData(sprite: Sprite, groupName: String?, brick: Brick) {
        for (userVariable in getUserVariables(sprite, brick)) {
            createInitialHashmap(groupName, BackpackListManager.getInstance().backpack.backpackedUserVariables)
            when {
        ...
        for (userList in getUserLists(sprite, brick)) {
            createInitialHashmap(groupName, BackpackListManager.getInstance().backpack.backpackedUserLists)
            when {

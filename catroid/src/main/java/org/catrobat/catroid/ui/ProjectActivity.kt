...
import org.catrobat.catroid.ui.fragment.PROJECT_OPTIONS_FRAGMENT_TAG
...
import org.catrobat.catroid.ui.recyclerview.fragment.SCENE_LIST_FRAGMENT_TAG
import org.catrobat.catroid.ui.recyclerview.fragment.SPRITE_LIST_FRAGMENT_TAG
...
class ProjectActivity : BaseCastActivity() {
    ...
    private fun loadFragment(fragmentPosition: Int) {
        ...
        when (fragmentPosition) {
            FRAGMENT_SCENES -> fragmentTransaction.replace(
                ...
                SceneListFragment(),
                SCENE_LIST_FRAGMENT_TAG
            )
            FRAGMENT_SPRITES -> fragmentTransaction.replace(
                R.id.fragment_container,
                SpriteListFragment(),
                SPRITE_LIST_FRAGMENT_TAG
            )
        ...
        when (item.itemId) {
            ...
            R.id.project_options -> supportFragmentManager.beginTransaction()
                .replace(
                    ...
                    PROJECT_OPTIONS_FRAGMENT_TAG
                )
                .addToBackStack(PROJECT_OPTIONS_FRAGMENT_TAG)
                .commit()
    ...
    public override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        handleRequestCode(requestCode, data)
    }

    private fun handleRequestCode(requestCode: Int, data: Intent?) {
        when (requestCode) {
            SPRITE_POCKET_PAINT -> {
                val uri = ImportFromPocketPaintLauncher(this).getPocketPaintCacheUri()
            ...
            SPRITE_CAMERA -> {
                val uri = ImportFromCameraLauncher(this).getCacheCameraUri()
                addSpriteFromUri(uri, Constants.JPEG_IMAGE_EXTENSION)
            }
    ...
    private fun addSpriteObjectFromUri(uri: Uri?, extension: String, isObject: Boolean) {
        ...
        if (isObject) {
            importProjectHelper = ImportProjectHelper(lookFileName, currentScene, this)
    ...
    fun handleAddSceneButton() {
        val currentProject = projectManager.currentProject
        val defaultSceneName = SceneController
            .getUniqueDefaultSceneName(resources, currentProject.sceneList)
    ...
    private fun handleAddSpriteButton() {
        ...
        dialogNewActorBinding.dialogNewLookObjectLibrary
            .setOnClickListener {
                ImportFormMediaLibraryLauncher(this, LIBRARY_OBJECT_URL)

...
class CatrobatLanguageParserVisitor(private val context: Context) : CatrobatLanguageParserVisitor<CatrobatLanguageBaseResult> {
    ...
    private data class BrickParameterInfo(
        val brick: Brick,
        val context: Context,
        val project: Project,
        val scene: Scene,
    ...
    override fun visitGlobals(ctx: CatrobatLanguageParser.GlobalsContext?): CatrobatLanguageBaseResult {
        ...
        ctx.variableOrListDeclaration().forEach {
            ...
            if (result is CatrobatLanguageListResult) {
                validateUniqueGlobalListName(result.listName)
                project.userLists.add(UserList(result.listName))
            } else if (result is CatrobatLanguageVariableResult) {
                validateUniqueGlobalVariableName(result.variableName)
    ...
    override fun visitMultiplayerVariables(ctx: CatrobatLanguageParser.MultiplayerVariablesContext?): CatrobatLanguageBaseResult {
        ...
        ctx.variableDeclaration().forEach {
            ...
            validateUniqueGlobalVariableName(variableName)
    ...
    private fun validateUniqueGlobalVariableName(name: String) {
        if (project.userVariables.any { variable -> variable.name == name } ||
            ...
            throw CatrobatLanguageParsingException("Variable $name must occur at most once.")
        }
    }

    private fun validateUniqueGlobalListName(name: String) {
        if (project.userLists.any { list -> list.name == name }) {
            throw CatrobatLanguageParsingException("List $name must occur at most once.")
        }
    }
    ...
    override fun visitLocalVariables(ctx: CatrobatLanguageParser.LocalVariablesContext?): CatrobatLanguageBaseResult {
        ...
        ctx.variableOrListDeclaration().forEach {
            val result = visitVariableOrListDeclaration(it)
            if (result is CatrobatLanguageListResult) {
                validateUniqueLocalListName(result.listName)
                currentSprite!!.userLists.add(UserList(result.listName))
            } else if (result is CatrobatLanguageVariableResult) {
                validateUniqueLocalVariableName(result.variableName)
    ...
    private fun validateUniqueLocalVariableName(name: String) {
        validateUniqueGlobalVariableName(name)
        if (currentSprite!!.userVariables.any { variable -> variable.name == name }) {
            throw CatrobatLanguageParsingException("Variable $name must occur at most once.")
        }
    }

    private fun validateUniqueLocalListName(name: String) {
        validateUniqueGlobalListName(name)
        if (currentSprite!!.userLists.any { list -> list.name == name }) {
            throw CatrobatLanguageParsingException("List $name must occur at most once.")
        }
    }

...
class SetSpeakingLanguageBrick : BrickBaseType(),
    ...
    override fun addActionToSequence(sprite: Sprite, sequence: ScriptSequenceAction) {
        sequence.addAction(
            sprite.actionFactory.createSetSpeakingLanguageAction(
    ...
    override fun getView(context: Context?): View {
        ...
        val supportedLocales = TextToSpeechHolder.instance.getSupportedLocales()
        languageObjectList.apply {
            supportedLocales
                .forEach { locale ->
                    val languageName = locale.getDisplayName(locale)
                    add(LanguageObject(languageName, locale.toLanguageTag()))
                }
        }

        with(
            BrickSpinner<LanguageObject>(
                R.id.brick_set_speaking_language_spinner,
                view,
                languageObjectList
            )
        ) {
            spinner = this
            setOnItemSelectedListener(this@SetSpeakingLanguageBrick)
    ...
    override fun getViewResource(): Int = R.layout.brick_set_speaking_language
    ...
    override fun addRequiredResources(requiredResourcesSet: Brick.ResourcesSet) {
        requiredResourcesSet.add(Brick.TEXT_TO_SPEECH)

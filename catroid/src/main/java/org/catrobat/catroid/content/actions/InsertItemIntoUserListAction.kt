...
class InsertItemIntoUserListAction : TemporalAction() {
    ...
    override fun update(percent: Float) {
        ...
        val value: Any? = if (formulaItemToInsert == null) {
            java.lang.Double.valueOf(0.0)
        } else {
            try {
                formulaItemToInsert?.interpretObject(scope)
            } catch (e: InterpretationException) {
                throw InterpretationException("Failed to interpret formulaItemToInsert", e)
            }
        }
        var indexToInsert = try {
            formulaIndexToInsert?.interpretInteger(scope) ?: 1
        } catch (e: InterpretationException) {
            throw InterpretationException("Failed to interpret formulaIndexToInsert", e)
        }
        indexToInsert--

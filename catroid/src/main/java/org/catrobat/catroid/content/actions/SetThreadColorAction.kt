...
class SetThreadColorAction : TemporalAction() {
    ...
    override fun update(delta: Float) {
        try {
            var colorStringInterpretation = "0xff0000"
            if (color != null) {
                colorStringInterpretation = color!!.interpretString(scope)
            }
            val red = "0x" + colorStringInterpretation.substring(1, 3)
            val green = "0x" + colorStringInterpretation.substring(3, 5)
            val blue = "0x" + colorStringInterpretation.substring(5, 7)
            ...
            Color.argb8888ToColor(
                colorInterpretation,
                android.graphics.Color.argb(0xFF, redInt, greenInt, blueInt)
            )

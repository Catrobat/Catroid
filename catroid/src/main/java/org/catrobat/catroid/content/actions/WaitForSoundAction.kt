/*
 * Catroid: An on-device visual programming system for Android devices
 * Copyright (C) 2010-2022 The Catrobat Team
 * (<http://developer.catrobat.org/credits>)
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * An additional term exception under section 7 of the GNU Affero
 * General Public License, version 3, is available at
 * http://developer.catrobat.org/license_additional_term
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */
package org.catrobat.catroid.content.actions

import androidx.annotation.VisibleForTesting
import org.catrobat.catroid.content.MediaPlayerWithSoundDetails
import org.catrobat.catroid.content.SoundFilePathWithSprite
import org.catrobat.catroid.io.SoundManager
import org.catrobat.catroid.pocketmusic.mididriver.MidiPlayer
import org.catrobat.catroid.pocketmusic.mididriver.MidiSoundManager

class WaitForSoundAction : WaitAction() {
    private var soundFilePath: String? = null
    private var soundManager = SoundManager.getInstance()
    private var midiSoundManager = MidiSoundManager.getInstance()
    private var soundStopped = false
    fun setSoundFilePath(soundFilePath: String?) {
        this.soundFilePath = soundFilePath
    }

    override fun update(percent: Float) {
        if (soundFilePath != null && midiSoundManager.startedSoundfilePaths.isNotEmpty()) {
            val spriteSoundFilePath = SoundFilePathWithSprite(soundFilePath, scope.sprite)
            val recentlyStarted = midiSoundManager.startedSoundfilePaths
            if (recentlyStarted.contains(spriteSoundFilePath) && !midiSoundManager.isSoundInSpritePlaying(
                    scope.sprite,
                    soundFilePath
                )
            ) {
                recentlyStarted.remove(spriteSoundFilePath)
                finish()
                soundStopped = true
                return
            }
        }
        if (soundFilePath != null && soundManager.recentlyStoppedSoundfilePaths.isNotEmpty()) {
            val spriteSoundFilePath = SoundFilePathWithSprite(soundFilePath, scope.sprite)
            val recentlyStopped = soundManager.recentlyStoppedSoundfilePaths
            if (recentlyStopped.contains(spriteSoundFilePath)) {
                recentlyStopped.remove(spriteSoundFilePath)
                finish()
                soundStopped = true
            }
        }
    }

    override fun end() {
        for (mediaPlayer in soundManager.mediaPlayers) {
            if (checkMediaPlayerValidity(mediaPlayer)) {
                restart()
                time = mediaPlayer.currentPosition.toFloat()
            }
        }
        for (midiPlayer in midiSoundManager.midiPlayers) {
            if (checkMidiPlayerValidity(midiPlayer)) {
                restart()
                time = midiPlayer.currentPosition.toFloat()
            }
        }
    }

    private fun checkMediaPlayerValidity(mediaPlayer: MediaPlayerWithSoundDetails): Boolean {
        if (!mediaPlayer.isPlaying || !(mediaPlayer.startedBySprite === scope.sprite)) {
            return false
        }
        if (mediaPlayer.pathToSoundFile != soundFilePath || soundStopped) {
            return false
        }
        return true
    }

    private fun checkMidiPlayerValidity(midiPlayer: MidiPlayer): Boolean {
        if (!midiPlayer.isPlaying || !(midiPlayer.startedBySprite === scope.sprite)) {
            return false
        }
        if (midiPlayer.pathToSoundFile != soundFilePath || soundStopped) {
            return false
        }
        return true
    }

    @VisibleForTesting
    fun setSoundManager(soundManager: SoundManager) {
        this.soundManager = soundManager
    }

    @VisibleForTesting
    fun setMidiSoundManager(midiSoundManager: MidiSoundManager) {
        this.midiSoundManager = midiSoundManager
    }

    companion object {
        val TAG: String? = WaitForSoundAction::class.java.simpleName
    }
}

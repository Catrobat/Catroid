...
private const val DEFAULT_COLOR = 25f
...
class ChangeColorByNAction : TemporalAction() {
    ...
    override fun update(delta: Float) {
        scope?.let {
            try {
                val newColor: Float = color?.interpretFloat(it) ?: DEFAULT_COLOR
                it.sprite.look.changeColorInUserInterfaceDimensionUnit(newColor)

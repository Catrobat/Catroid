...
public class LookDataHistory extends MediaHistory {
	private static final Map<Sprite, LookDataHistory> INSTANCE = new HashMap<>();

	public static LookDataHistory getInstance(Sprite sprite) {
		if (!INSTANCE.containsKey(sprite)) {
			INSTANCE.put(sprite, new LookDataHistory());
		}
	...
	public static void applyChanges() {
		...
		if (StorageHandler.getInstance().getLookFileList(projectName) == null) {
			INSTANCE.clear();
			return;
		}

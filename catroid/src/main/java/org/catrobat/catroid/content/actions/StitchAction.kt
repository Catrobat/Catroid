...
class StitchAction : TemporalAction() {
    ...
    override fun update(delta: Float) {
        ...
        val x = existingSprite.look.xInUserInterfaceDimensionUnit
        val y = existingSprite.look.yInUserInterfaceDimensionUnit
        StageActivity.stageListener.embroideryPatternManager.addStitchCommand(
            DSTStitchCommand(
                x,
                y,
                existingSprite.look.zIndex,
                existingSprite,
                existingSprite.embroideryThreadColor
            )
        )
        existingSprite.runningStitch.setStartCoordinates(x, y)

...
public class DigitalMessageWriter implements IMessageWriter<DigitalMessage> {
    ...
    public void write(DigitalMessage message, BluetoothDevice btDevice) {
        byte[] buffer = new byte[3];

        buffer[0] = (byte) (COMMAND | ENCODE_CHANNEL(message.getPort()));
        buffer[1] = (byte) LSB(message.getValue());
        buffer[2] = (byte) MSB(message.getValue());

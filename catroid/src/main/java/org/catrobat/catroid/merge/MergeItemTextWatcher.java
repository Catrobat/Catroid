...
public class MergeItemTextWatcher<T extends Nameable> extends TextInputDialog.TextWatcher {
	private List<T> scope;

	public MergeItemTextWatcher(List<T> scope) {
		this.scope = scope;
	}
	...
	public String validateInput(String input, Context context) {
		...
		} else if (!isNameUnique(input) && Utils.checkIfProjectExistsOrIsDownloadingIgnoreCase(input)) {

...
private const val USE_FAKE_SPLIT_INSTALL_MANAGER = false
...
// 1. Set USE_FAKE_SPLIT_INSTALL_MANAGER = true
// 2. Build > Generate signed bundles > select AAB
// 3. bundletool build-apks --local-testing --bundle catroid/catroid/debug/catroid-catroid-debug.aab --output app.apks
// 4. bundletool install-apks --apks app.apks
...
object MachineLearningUtil {
    ...
    @JvmStatic
    fun getVisualDetectionHandler() =
        getObjectInstance<VisualDetectionHandler>("VisualDetectionHandler")

    // NOTE: Use the fakeSplitInstallManager if you want to test the lazy loading with
    // a simulated network error locally with the simulator
    // see: https://star-zero.medium.com/an-easy-way-to-test-offline-dynamic-delivery-9b6909b62bcb
    private fun createSplitInstallManager(context: Context): SplitInstallManager {
        if (USE_FAKE_SPLIT_INSTALL_MANAGER) {
            val fakeSplitInstallManager = FakeSplitInstallManagerFactory.create(
                context,
    ...
    private fun <T> getObjectInstance(name: String): T? {
        ...
        if (loadingState == LoadingState.NOT_LOADED) {
            ...
            if (context != null) {
                val splitInstallManager = createSplitInstallManager(context)
        ...
        return try {
            ...
            Log.e(
                ...
                "Could not get class for name '$MODULE_PATH.$name'",
                exception
            )
            null
        }
    ...
    private fun loadModule() {
        ...
        val notificationData =
            statusBarNotificationManager.createMLModuleDownloadNotification(context)

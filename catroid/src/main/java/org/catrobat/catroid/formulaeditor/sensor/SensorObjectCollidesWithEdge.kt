...
open class SensorObjectCollidesWithEdge : Sensor {

    override fun getSensorValue(): Double {
        return tryCalculateCollidesWithEdge(
            SensorHandler.currentSprite.look, StageActivity.stageListener,
            SensorHandler.currentProject.screenRectangle
        ) { p: Array<Polygon>, r: Rectangle? -> CollisionDetection.collidesWithEdge(p, r) }
    }

    protected fun tryCalculateCollidesWithEdge(
        look: Look,
        stageListener: StageListener?,
        screen: Rectangle?,
        collisionDetector: (Array<Polygon>, Rectangle?) -> Boolean
    ): Double {
        return if (stageListener?.firstFrameDrawn == true) {
            Conversions.booleanToDouble(
                collisionDetector(look.currentCollisionPolygon, screen)
            )
        } else {
            Conversions.FALSE
        }

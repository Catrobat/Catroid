...
public class DefaultScrollerViewProvider extends ScrollerViewProvider {
	...
	public View provideHandleView(ViewGroup container) {
		...
		int verticalInsetBottom = getContext().getResources().getDimensionPixelSize(R.dimen
				.pocketmusic_fastscroller_handle_inset_bottom);
		...
		InsetDrawable handleBg = new InsetDrawable(ContextCompat.getDrawable(getContext(), R.drawable
				.pocketmusic_fastscroller_handle), horizontalInset, verticalInsetTop, horizontalInset, verticalInsetBottom);
		handle.setBackground(handleBg);

		int handleWidth = getContext().getResources().getDimensionPixelSize(R.dimen.pocketmusic_fastscroller_handle_clickable_width);
		int handleHeight = getContext().getResources().getDimensionPixelSize(R.dimen.pocketmusic_fastscroller_handle_height);
	...
	public View provideBubbleView(ViewGroup container) {
		bubble = LayoutInflater.from(getContext()).inflate(R.layout.pocketmusic_fastscroller_bubble, container, false);
		return bubble;
	}
	...
	protected ViewBehavior provideHandleBehavior() {
		return new DefaultHandleBehavior(new VisibilityAnimationManager.Builder(handle).withHideDelay(2000)
				.withHideAnimator(R.animator.pocketmusic_fastscroller_fade_out).withShowAnimator(R.animator.pocketmusic_fastscroller_fade_in).build());
	}

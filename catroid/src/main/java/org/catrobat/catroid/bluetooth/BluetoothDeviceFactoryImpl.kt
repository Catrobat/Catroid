...
class BluetoothDeviceFactoryImpl : BluetoothDeviceFactory {
    override fun <T : BluetoothDevice?> createDevice(
        service: Class<T>,
        applicationContext: Context
    ): BluetoothDevice? {
        if (service == BluetoothDevice.LEGO_NXT) {
            return LegoNXTImpl(applicationContext)
        }
        if (service == BluetoothDevice.LEGO_EV3) {
            return LegoEV3Impl(applicationContext)
        }
        if (service == BluetoothDevice.PHIRO) {
            return PhiroImpl()
        }
        return if (service == BluetoothDevice.ARDUINO) {
            ArduinoImpl()
        } else null
    }
}
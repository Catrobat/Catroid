...
public class SetVolumeToActionTest {
	...
	public void testBrickWithStringFormula() {
		sprite.getActionFactory().createSetVolumeToAction(sprite,  new SequenceAction(),new Formula(String.valueOf(VOLUME))).act(1.0f);
		assertEquals(VOLUME, SoundManager.getInstance().getVolume());

		sprite.getActionFactory().createSetVolumeToAction(sprite,  new SequenceAction(),new Formula(NOT_NUMERICAL_STRING)).act(1.0f);
		assertEquals(VOLUME, SoundManager.getInstance().getVolume());
	}

...
public class ShowBubbleActorTest {
	...
	private MockedStatic<Bitmap> bitmapMock;
	private MockedConstruction<Image> imageMock;
	private MockedConstruction<Pixmap> pixmapMock;
	private MockedConstruction<Texture> textureMock;
	...
	public void setUp() throws Exception {
		...
		bitmapMock = mockStatic(Bitmap.class);
		bitmapMock.when(() -> Bitmap.createBitmap(anyInt(), anyInt(),
				Mockito.any(Bitmap.Config.class))).thenReturn(mock(Bitmap.class));

		imageMock = mockConstruction(Image.class,
				(mock, context) -> when(mock.getWidth()).thenReturn(IMAGE_WIDTH));
		pixmapMock = mockConstruction(Pixmap.class);
		textureMock = mockConstruction(Texture.class);
	}

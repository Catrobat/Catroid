...
class ProjectsCategoriesSyncWorkerTest {

    private val projectsCategoriesSync = mockk<ProjectsCategoriesSync>(relaxed = true)
    ...
    fun setUp() {
        ...
        startKoin {
            modules(module {
                single<ProjectsCategoriesSync> { projectsCategoriesSync }
            })
        }
    }
    ...
    private fun buildWorker(runAttemptCount: Int = 0) =
        TestListenableWorkerBuilder<ProjectsCategoriesSyncWorker>(context)
            .setRunAttemptCount(runAttemptCount)
            .build()

    private fun givenSyncFails() {
        coEvery { projectsCategoriesSync.sync(any()) } throws
    ...
    fun testSyncSuccessReturnsSuccess() = runBlocking {
        ...
        coVerify(exactly = 1) { projectsCategoriesSync.sync(any()) }
    }

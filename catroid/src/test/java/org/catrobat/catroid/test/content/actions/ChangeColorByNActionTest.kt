...
class ChangeColorByNActionTest {
    ...
    fun setUp() {
        ...
        sprite.actionFactory.createSetColorAction(
            sprite, SequenceAction(), Formula(INITIALIZED_VALUE)
        ).act(1.0f)
    }
    ...
    fun testNormalBehavior() {
        ...
        sprite.actionFactory.createChangeColorByNAction(
            sprite, SequenceAction(), Formula(DELTA)
        ).act(1.0f)
        assertEquals(
            INITIALIZED_VALUE + DELTA, sprite.look.colorInUserInterfaceDimensionUnit
        )
    ...
    fun testNullSprite() {
        sprite.actionFactory.createChangeColorByNAction(
            null, SequenceAction(), Formula(DELTA)
        ).act(1.0f)
    }
    ...
    fun testBrickWithStringFormula() {
        sprite.actionFactory.createChangeColorByNAction(
            sprite, SequenceAction(), Formula(DELTA.toString())
        ).act(1.0f)
        assertEquals(
            INITIALIZED_VALUE + DELTA, sprite.look.colorInUserInterfaceDimensionUnit
        )
    ...
    fun testWrapAround() {
        sprite.actionFactory.createSetColorAction(
            sprite, SequenceAction(), Formula(199.0f)
        ).act(1.0f)

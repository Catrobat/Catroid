...
public class DroneMoveBackwardAction extends TemporalAction {

	private static final String TAG = DroneMoveBackwardAction.class.getSimpleName();
	...
	protected void update(float percent) {
		...
		//DroneServiceWrapper.getInstance().getDroneService().moveBackward(DroneControlService.POWER);
		DroneServiceWrapper.getInstance().getDroneService()
				.moveBackward((float) powerInPercent.interpretInteger(sprite) / 100);
	}
	...
	protected void end() {
		...
		DroneServiceWrapper.getInstance().getDroneService().moveBackward(0);
	}

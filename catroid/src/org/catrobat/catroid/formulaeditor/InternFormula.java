...
public class InternFormula {
	...
	private InternToken getSelectedToken() {
		boolean validSelectedToken = (internFormulaTokenSelection == null ||
				internFormulaTokenSelection.getTokenSelectionType() != TokenSelectionType.USER_SELECTION);
		if (validSelectedToken) {
			return null;
		}
		int currentIndex = 0;
		for (InternToken token : internTokenFormulaList) {
			boolean validTokenType = token.getInternTokenType() == InternTokenType.STRING
